...
struct kp_cam_buf_io_cfg {
    ...
    unsigned int format, color_space, color_pattern, bpp, rotation, resource_type;
...
/* 0=idle 1=armed 2=RDI0 BUF DONE,等待g 3=已抓取,等待w 4=已写盘 */
static volatile char capture_status;

static volatile unsigned int cnt_prepare, cnt_done, cnt_get, cnt_copy_ok, cnt_copy_fail;
...
/* prepare阶段已经dma_buf_get()成功,g直接复用这个引用,不重新get */
static struct dma_buf *capture_dmabuf;

static volatile unsigned int capture_width, capture_height, capture_stride;
static volatile unsigned int capture_slice, capture_format, capture_offset;

static volatile unsigned long cached_len, cached_request;
...
static void before_prepare(hook_fargs2_t *args, void *udata)
{
    ...
    if (capture_dmabuf)   // 这一轮已经抓到目标buffer,不再接受新PREP
        return;
    ...
    io_cfg = (struct kp_cam_buf_io_cfg *)(
        (unsigned char *)packet->payload + packet->io_configs_offset);
    ...
    for (i = 0; i < packet->num_io_configs; i++) {
        ...
        // 这里拿一次引用,后续g直接使用capture_dmabuf
        dmabuf = p_dma_buf_get(fd);
        if (!dmabuf) {
            pr_info("cam-raw-dump: PREP dma_buf_get FAILED fd=%d\n", fd);
            cnt_copy_fail++;
            return;
        }
        ...
        capture_width = io_cfg[i].planes[0].width;
        capture_height = io_cfg[i].planes[0].height;
        capture_stride = io_cfg[i].planes[0].plane_stride;
        capture_slice = io_cfg[i].planes[0].slice_height;
        capture_format = io_cfg[i].format;
        capture_offset = io_cfg[i].offsets[0];

        cnt_prepare++;

        pr_info("cam-raw-dump: PREP saved req=%llu mem=0x%x fd=%d dmabuf=%p "
                "w=%u h=%u stride=%u slice=%u offset=%u format=%u\n",
                capture_request, capture_handle, capture_fd, capture_dmabuf,
                capture_width, capture_height, capture_stride,
                capture_slice, capture_offset, capture_format);
        return;
    }
}
...
static void before_buf_done(hook_fargs2_t *args, void *udata)
{
    ...
    event_info = (struct kp_cam_isp_hw_event_info *)args->arg1;
    if (!event_info || !event_info->event_data)
        return;

    compdone = (struct kp_cam_isp_hw_compdone_event_info *)event_info->event_data;
    ...
    for (i = 0; i < num_res; i++) {
        ...
        // BUF DONE只负责确认时机,绝不在这里访问dma_buf
        capture_status = 2;

        pr_info("cam-raw-dump: RDI0 DONE hw=%u last_addr=0x%x req=%llu mem=0x%x fd=%d "
                "dmabuf=%p w=%u h=%u stride=%u slice=%u offset=%u format=%u\n",
                event_info->hw_idx, compdone->last_consumed_addr[i],
                capture_request, capture_handle, capture_fd, capture_dmabuf,
                capture_width, capture_height, capture_stride,
                capture_slice, capture_offset, capture_format);
        return;
...
static int capture_frame_from_control_context(void)
{
    ...
    if (!p_dma_buf_begin_cpu_access || !p_dma_buf_end_cpu_access ||
        !p_dma_buf_vmap || !p_dma_buf_vunmap)
        return -1;

    len = (unsigned long)capture_stride *
          (unsigned long)(capture_slice ? capture_slice : capture_height);
    offset = (unsigned long)capture_offset;

    if (!len || len > DYNAMIC_BUF_SIZE || offset > DYNAMIC_BUF_SIZE ||
        offset + len > DYNAMIC_BUF_SIZE) {
        pr_info("cam-raw-dump: invalid capture len=%lu offset=%lu\n", len, offset);
        cnt_copy_fail++;
        return -1;
    }

    pr_info("cam-raw-dump: GET capture saved_dmabuf=%p fd=%d req=%llu len=%lu offset=%lu\n",
            capture_dmabuf, capture_fd, capture_request, len, offset);

    rc = p_dma_buf_begin_cpu_access(capture_dmabuf, 0);
    if (rc) {
        pr_info("cam-raw-dump: begin_cpu_access rc=%d\n", rc);
        cnt_copy_fail++;
        return -1;
    }

    vaddr = p_dma_buf_vmap(capture_dmabuf);
    if (!vaddr) {
        pr_info("cam-raw-dump: vmap failed\n");
        p_dma_buf_end_cpu_access(capture_dmabuf, 0);
        cnt_copy_fail++;
        return -1;
    }

    pr_info("cam-raw-dump: copy source=%lx offset=%lu len=%lu\n",
            (unsigned long)vaddr, offset, len);

    // 真正的整帧复制,不重新dma_buf_get(capture_fd)
    memcpy(dyn_buf, (unsigned char *)vaddr + offset, len);

    pr_info("cam-raw-dump: memcpy done len=%lu\n", len);

    p_dma_buf_vunmap(capture_dmabuf, vaddr);
    p_dma_buf_end_cpu_access(capture_dmabuf, 0);
    ...
    pr_info("cam-raw-dump: CAPTURE OK req=%lu fd=%d len=%lu w=%u h=%u stride=%u "
            "slice=%u offset=%u format=%u\n",
            cached_request, capture_fd, cached_len, capture_width, capture_height,
            capture_stride, capture_slice, capture_offset, capture_format);
...
static int write_cached_frame_to_disk(void)
{
    ...
    file = p_filp_open("/data/local/tmp/rdi0.raw", O_CREAT | O_WRONLY | O_TRUNC, 0644);
    if (is_err_ptr(file)) {
        pr_err("cam-raw-dump: open failed\n");
        return -1;
    }

    written = p_kernel_write(file, dyn_buf, cached_len, &pos);
    p_filp_close(file, NULL);

    pr_info("cam-raw-dump: written=%ld expected=%lu\n", written, cached_len);
...
static long cam_kpm_init(const char *args, const char *event, void *reserved)
{
    ...
    p_dma_buf_get = (void *)kallsyms_lookup_name("dma_buf_get");
    p_dma_buf_put = (void *)kallsyms_lookup_name("dma_buf_put");
    p_dma_buf_begin_cpu_access = (void *)kallsyms_lookup_name("dma_buf_begin_cpu_access");
    p_dma_buf_end_cpu_access = (void *)kallsyms_lookup_name("dma_buf_end_cpu_access");
    p_dma_buf_vmap = (void *)kallsyms_lookup_name("dma_buf_vmap");
    p_dma_buf_vunmap = (void *)kallsyms_lookup_name("dma_buf_vunmap");

    addr_prepare = kallsyms_lookup_name("cam_ife_mgr_prepare_hw_update");
    addr_buf_done = kallsyms_lookup_name("cam_ife_hw_mgr_handle_hw_buf_done");

    pr_info("cam-raw-dump: prepare=%lx buf_done=%lx get=%lx begin=%lx end=%lx vmap=%lx vunmap=%lx\n",
            addr_prepare, addr_buf_done,
            (unsigned long)p_dma_buf_get, (unsigned long)p_dma_buf_begin_cpu_access,
            (unsigned long)p_dma_buf_end_cpu_access,
            (unsigned long)p_dma_buf_vmap, (unsigned long)p_dma_buf_vunmap);

    if (!p_filp_open || !p_kernel_write || !p_filp_close || !p_vmalloc || !p_vfree ||
        !p_dma_buf_get || !p_dma_buf_put || !p_dma_buf_begin_cpu_access ||
        !p_dma_buf_end_cpu_access || !p_dma_buf_vmap || !p_dma_buf_vunmap ||
        !addr_prepare || !addr_buf_done)
        return -1;
    ...
    if (hook_wrap2((void *)addr_buf_done, before_buf_done, NULL, NULL)) {
...
static long cam_kpm_control0(const char *args, char __user *out_msg, int outlen)
{
    ...
    if (args[0] == 'c') {
        release_capture_dmabuf();   // 清理上一轮引用

        capture_status = 1;
        cached_len = 0;
        cached_request = 0;
        cnt_prepare = cnt_done = cnt_get = cnt_copy_ok = cnt_copy_fail = 0;
        capture_width = capture_height = capture_stride = 0;
        capture_slice = capture_format = capture_offset = 0;
        ...
        if (capture_status != 2) {
            pr_info("cam-raw-dump: no RDI0 DONE frame\n");
            compat_copy_to_user(out_msg, "no_done", 8);
            return 0;
        }

        if (capture_frame_from_control_context() == 0)
            compat_copy_to_user(out_msg, "capture_ok", 11);
        else
            compat_copy_to_user(out_msg, "capture_fail", 13);

    } else if (args[0] == 'w') {
        if (capture_status != 3 || !cached_len) {
            pr_info("cam-raw-dump: no captured frame\n");
            compat_copy_to_user(out_msg, "no_frame", 9);
            return 0;
        }
        ...
        pr_info("cam-raw-dump: STOP status=%d prepare=%u done=%u get=%u copy_ok=%u "
                "copy_fail=%u len=%lu req=%lu fd=%d dmabuf=%p\n",
                capture_status, cnt_prepare, cnt_done, cnt_get, cnt_copy_ok,
                cnt_copy_fail, cached_len, cached_request, capture_fd, capture_dmabuf);
